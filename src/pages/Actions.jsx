...
import { CheckCircle, Clock, CreditCard, ArrowRight, AlertCircle, Search, Filter } from 'lucide-react';
...
const Actions = () => {
    ...
    // Define actionable stages
    const actionableStages = ['Demo Scheduled', 'Meeting Scheduled', 'Ready for Payment', 'Proposal Sent'];

    // Filter leads requiring action
    const actionableLeads = leads.filter(lead =>
        actionableStages.includes(lead.stage)
    );
        ...
        switch (stage) {
            ...
            case 'Meeting Scheduled':
                return {
                    ...
                    description: 'Scheduled meeting requires attendance'
                };
            case 'Ready for Payment':
                return {
                    ...
                    description: 'Client is ready to transact'
                };
            case 'Proposal Sent':
                return {
                    icon: AlertCircle,
            ...
            <div className="grid gap-4">
                {actionableLeads.length === 0 ? (
                    ...
                    actionableLeads.map(lead => {
                        const config = getActionConfig(lead.stage);
                            ...
                            <div key={lead.id} className="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-md transition-shadow flex flex-col sm:flex-row items-start sm:items-center gap-6">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-center gap-3 mb-1">
                                        ...
                                        <span className={`px-2 py-0.5 rounded-full text-xs font-medium border ${config.color.replace('bg-', 'border-').replace('/10', '/20')} ${config.color.split(' ')[0]}`}>
                                            {lead.stage}
                                        </span>
                                    </div>
                                    ...
                                    <p className="text-xs text-gray-400 mt-1">Last active: {new Date(lead.createdAt).toLocaleDateString()}</p>
                                </div>
